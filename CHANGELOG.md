...
Initial standalone repo. The **Surfn-Tela-Black** folder icons were taken from the Tela icon theme
(`vinceliuice/Tela-icon-theme`) and surfn-ified so they ship as `surfn-tela-black-icons-git`, depending on the
base `surfn-icons-git`.

### Technical Details

- Only the `places` context is shipped (folders); everything else inherits Surfn.
- `index.theme`: `Name=Surfn Tela Black`, `Inherits=Surfn,hicolor`.
- Cross-context symlinks dereferenced so no broken links ship; `icon-theme.cache` excluded.

### Files Modified

- Initial scaffold + usr/share/icons/Surfn-Tela-Black/
